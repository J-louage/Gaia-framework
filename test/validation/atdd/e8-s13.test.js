...
describe("E8-S13: Val Permanent Memory and Ground Truth Seed", () => {
  ...
  describe("AC3: Ground truth documentation sections", () => {
    it("test_ac3_ground_truth_documentation_sections — contains correction, inventory, pattern, structure, and coverage sections", () => {
      ...
      for (const section of requiredDocSections) {
        ...
        expect(
          found,
          `Missing documentation section: ${section.name}`,
        ).toBe(true);
      }
    });
  });
  ...
  describe("AC4: Val token budget", () => {
    it("test_ac4_val_token_budget — 200K token budget documented in memory config", () => {
      ...
      expect(existsSync(MEMORY_CONFIG), "_memory/config.yaml must exist").toBe(
        true,
      );
      ...
      expect(
        configContent,
        "config.yaml must reference validator agent",
      ).toMatch(/validator/i);
      expect(
        configContent,
        "config.yaml must contain 200000 token budget",
      ).toMatch(/200000/);
      ...
      if (existsSync(GROUND_TRUTH)) {
        ...
        expect(
          gtContent,
          "ground-truth.md frontmatter must contain token_budget: 200000",
        ).toMatch(/token_budget:\s*200000/);
      }
    });
  });
});
